...
class AppStore extends Component {
  ...
  render() {
    ...
    const filterList = appsList.filter(each => activeTab === each.category)
    const fil = filterList.filter(item =>
      item.appName.toUpperCase().includes(searchValue.toUpperCase()),
    )
    ...
    return (
      <div className="cont-cont">
        ...
        <ul className="tabs-para-list">
          {fil.map(each => (
            <Item det={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
